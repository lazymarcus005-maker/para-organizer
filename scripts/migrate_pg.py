...
# ── Table mapping: (sqlite_table, pg_table, columns, transform_fn) ──────────

def _serialize(val: Any) -> Any:
    """Convert Python objects to JSON strings for TEXT columns."""
...
def _transform_note(row: dict) -> dict:
    ...
    row = dict(row)
    # tags: stored as JSON string in SQLite, JSONB in PG
    if isinstance(row.get("tags"), str):
        try:
            row["tags"] = json.loads(row["tags"])
        except (json.JSONDecodeError, TypeError):
            row["tags"] = []
    # source_metadata: stored as JSON string in SQLite
    if isinstance(row.get("source_metadata"), str):
        try:
            row["source_metadata"] = json.loads(row["source_metadata"])
        except (json.JSONDecodeError, TypeError):
            row["source_metadata"] = {}
    # recurrence: stored as JSON string in SQLite
    if isinstance(row.get("recurrence"), str):
        try:
            row["recurrence"] = json.loads(row["recurrence"])
        except (json.JSONDecodeError, TypeError):
            row["recurrence"] = None
    # embedding: stored as bytes in sqlite-vec, skip for now (re-embed)
...
async def main() -> None:
    ...
    # Extract DSN from the async URL
